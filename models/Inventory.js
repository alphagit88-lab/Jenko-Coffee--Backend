  ...
  static async findAll(customerId = null) {
    const query = `
      SELECT 
        ...
        (SELECT COALESCE(SUM(quantity), 0) FROM inventory WHERE item_id = i.id) as warehouse_quantity,
        (SELECT COALESCE(SUM(quantity), 0) FROM salesperson_inventory WHERE item_id = i.id) as salesperson_quantity,
        ((SELECT COALESCE(SUM(quantity), 0) FROM inventory WHERE item_id = i.id) + (SELECT COALESCE(SUM(quantity), 0) FROM salesperson_inventory WHERE item_id = i.id)) as total_quantity,
        COALESCE((SELECT MIN(reorder_level) FROM inventory WHERE item_id = i.id), 10) as reorder_level,
        COALESCE((
